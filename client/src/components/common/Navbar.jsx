    ...
    <nav className="top-nav" dir="rtl">
      <div className="nav-right">
        <div className="logo-wrap">
          <img src="/logo.png" alt="לוגו" className="nav-logo" />
        </div>
      </div>
      ...
      <div className="nav-left">
        <div ref={menuRef} className="mobile-menu">
          <button className="hamburger" onClick={() => setOpenMenu((s) => !s)} aria-label="תפריט">☰</button>
          {openMenu && (
            <div className="mobile-menu-pop">
              <Link to="/home" onClick={() => setOpenMenu(false)}>דף הבית</Link>
              <Link to="/places" onClick={() => setOpenMenu(false)}>טיולים</Link>
              <Link to="/gallery" onClick={() => setOpenMenu(false)}>גלריה</Link>
            </div>
          )}
        </div>
        ...
        <div ref={userRef} className="user-area">
          {!user ? (
            <Link to="/login" className="auth-link">התחברות/הרשמה</Link>
          ) : (
            <div className="user-dropdown">
              <button className="user-btn" onClick={() => setOpenUser((s) => !s)}>
                שלום, {user.username}
              </button>
              {openUser && (
